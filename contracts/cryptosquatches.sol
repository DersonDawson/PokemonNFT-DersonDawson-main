...
contract CryptoSquatches is ERC721, Ownable {
     ...
     */
    function reserveCryptoSquatches(address _to, uint256 _numberOfTokens)
        external
        onlyOwner
    {
        ...
        returns (string memory)
    {
        ...
        if (startingIndex != 0 && block.timestamp >= REVEAL_TIMESTAMP) {
            string memory base = baseURI();
            string memory tokenURIWithOffset = uint2str(
                ((tokenId + startingIndex) % MAX_CRYPTO_SQUATCH_SUPPLY)
            );
    ...
    function mintCryptoSquatches(uint256 numberOfTokens) external payable {
        ...
        require(
            totalSupply().add(numberOfTokens) <= MAX_CRYPTO_SQUATCH_SUPPLY,
        ...
        if (
            ...
            (totalSupply() == MAX_CRYPTO_SQUATCH_SUPPLY ||
